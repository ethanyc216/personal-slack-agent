...
def test_watcher_settings_load_from_watcher_section(tmp_path):
    ...
    config_path.write_text(
        ...
        ultimate_search_workers = 1
        configured_channel_workers = 2
        runtime_backfill_workers = 3
    ...
    assert config.watcher.ultimate_search_workers == 1
    assert config.watcher.configured_channel_workers == 2
    assert config.watcher.runtime_backfill_workers == 3
...
def test_dump_config_emits_orchestrator_and_watcher_sections(tmp_path):
    ...
    assert "ultimate_search_workers = 1" in rendered
    assert "configured_channel_workers = 2" in rendered
    assert "runtime_backfill_workers = 3" in rendered
