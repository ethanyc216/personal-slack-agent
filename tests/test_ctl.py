...
def test_doctor_reports_api_test_error_detail(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("HOME", str(tmp_path))
    workspace_root = tmp_path / "work"
    workspace_root.mkdir()
    config_file = tmp_path / ".config" / "personal-slack-agent" / "bob.toml"
    ...
    assert exit_code == 0
    assert "workspace[bob_company].api_test: False" in captured.out
    assert (
        "workspace[bob_company].api_test_error: "
        "ssl_cert_verification_failed: unable to get local issuer certificate"
    ) in captured.out
